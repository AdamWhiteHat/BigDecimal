...
public class TestBigDecimalFunctions
{
	...
	public void TestRounding002()
	{
		...
		ClassicAssert.AreEqual(expected, actual1, $"{expected} != {actual1}");
		ClassicAssert.AreEqual(expected, actual2, $"{expected} != {actual2}");
	}
	...
	private static void Test(decimal value, int precision, RoundingStrategy roundingStrategy, string expected)
	{
		...
		ClassicAssert.AreEqual(expected, actual.ToString(), $"Value: {value}\tRounding: {Enum.GetName(typeof(RoundingStrategy), roundingStrategy)}\tExpected: {expected}\tActual: {actual}");
	}
