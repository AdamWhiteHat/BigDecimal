...
public readonly record struct BigDecimal : IComparable, IComparable<BigDecimal>, IComparable<Int32>, IComparable<Int32?>, IComparable<Decimal>, IComparable<Double>, IComparable<Single>
{
	...
	private static Lazy<BigDecimal> MaxBigDecimalForDecimal => new Lazy<BigDecimal>( static () => Decimal.MaxValue, true );

	private static Lazy<BigDecimal> MaxBigDecimalForSingle => new Lazy<BigDecimal>( static () => Single.MaxValue, true );

	private static Lazy<BigDecimal> MaxBigDecimalForDouble => new Lazy<BigDecimal>( static () => Double.MaxValue, true );

	private static Lazy<BigDecimal> MaxBigDemicalForInt32 => new Lazy<BigDecimal>( static () => Int32.MaxValue, true );

	private static Lazy<BigDecimal> MaxBigDemicalForUInt32 => new Lazy<BigDecimal>( static () => UInt32.MaxValue, true );
	...
	int IComparable.CompareTo( Object? obj )
	{
		...
		if ( obj is not BigDecimal ) { throw new ArgumentException( String.Format( LanguageResources.Argument_must_be_of_type__0_, nameof( BigDecimal ) ), nameof( obj ) ); }
	...
	public static BigDecimal Csch( BigDecimal radians, int precision )
	{
		...
		if ( input.IsZero() )
		{
			throw new ArithmeticException( "The hyperbolic cosecant of zero is undefined." );
		}
